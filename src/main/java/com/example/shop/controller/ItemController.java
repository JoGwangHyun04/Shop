...
public class ItemController {
    ...
    public String itemNew(@Valid ItemFormDto itemFormDto, BindingResult bindingResult,
        ...
        if(itemImgFileList.get(0).isEmpty() && itemFormDto.getId() == null) { //상품등록시 첫번째이미지없으면 에러메시지 및
                                                                                // 상품등록페이지로 전환
            model.addAttribute("errorMessage","첫번째 상품이미지는 필수입력값입니다.");
            return "item/itemForm";
        }

        try{
            itemService.saveItem(itemFormDto, itemImgFileList); //상품저장로직을 호출
        } catch (Exception e){
    ...
    public String itemUpdate(@Valid ItemFormDto itemFormDto, BindingResult bindingResult,
        ...
        if(itemImgFileList.get(0).isEmpty() && itemFormDto.getId() == null){
            model.addAttribute("errorMessage","첫번째 상품 이미지는 필수입력값입니다.");
            return "item/itemForm";
        }
